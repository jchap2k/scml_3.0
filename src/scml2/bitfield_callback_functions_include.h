...
inline void set_read_callback(bitfield<DT>& bitfield, C* c, typename bitfield_untimed_read_adapter<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&bitfield, CONFIGURATION_ERROR)
...
template <typename DT>
inline void set_clear_on_write(bitfield<DT>& bitfield, SyncType syncType  = AUTO_SYNCING) {
  bitfield.set_write_callback(create_bitfield_write_callback<DT>(create_bitfield_clear_on_write_adapter<false>(bitfield_null_adapter<DT>(), bitfield), "<clear_on_write>", syncType));
}

template <typename DT, typename C>
inline void set_clear_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_write_adapter<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  assert(c);
  bitfield.set_write_callback(create_bitfield_write_callback<DT>(create_bitfield_clear_on_write_adapter<false>(create_bitfield_write_adapter<DT>(*c, cb), bitfield), cbName, syncType));
}

template <typename DT, typename C>
inline void set_clear_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_untimed_write_adapter<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&bitfield, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << bitfield.get_name() << "'"
...
template <typename DT, typename C>
inline void set_clear_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_write_adapter_tagged<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  assert(c);
  bitfield.set_write_callback(create_bitfield_write_callback<DT>(create_bitfield_clear_on_write_adapter<false>(create_bitfield_write_adapter<DT>(*c, cb, id), bitfield), cbName, syncType));
}

template <typename DT, typename C>
inline void set_clear_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_untimed_write_adapter_tagged<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&bitfield, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << bitfield.get_name() << "'"
...
template <typename DT, typename C>
inline void set_clear_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_mask_write_adapter<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  assert(c);
  bitfield.set_write_callback(create_bitfield_write_callback<DT>(create_bitfield_clear_on_write_adapter<false>(create_bitfield_write_adapter<DT>(*c, cb), bitfield), cbName, syncType));
}

template <typename DT, typename C>
inline void set_clear_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_untimed_mask_write_adapter<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&bitfield, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << bitfield.get_name() << "'"
...
template <typename DT, typename C>
inline void set_clear_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_mask_write_adapter_tagged<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  assert(c);
  bitfield.set_write_callback(create_bitfield_write_callback<DT>(create_bitfield_clear_on_write_adapter<false>(create_bitfield_write_adapter<DT>(*c, cb, id), bitfield), cbName, syncType));
}

template <typename DT, typename C>
inline void set_clear_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_untimed_mask_write_adapter_tagged<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&bitfield, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << bitfield.get_name() << "'"
...
template <typename DT, typename C>
inline void set_post_clear_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_post_write_adapter<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&bitfield, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << bitfield.get_name() << "'"
  ...
  assert(c);
  bitfield.set_write_callback(create_bitfield_write_callback<DT>(create_bitfield_clear_on_write_adapter<true>(create_bitfield_post_write_adapter<DT>(*c, cb), bitfield), cbName, syncType));
}

template <typename DT, typename C>
inline void set_post_clear_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_post_write_adapter_tagged<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&bitfield, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << bitfield.get_name() << "'"
  ...
  assert(c);
  bitfield.set_write_callback(create_bitfield_write_callback<DT>(create_bitfield_clear_on_write_adapter<true>(create_bitfield_post_write_adapter<DT>(*c, cb, id), bitfield), cbName, syncType));
}
...
template <typename DT>
inline void set_set_on_write(bitfield<DT>& bitfield, SyncType syncType  = AUTO_SYNCING) {
  bitfield.set_write_callback(create_bitfield_write_callback<DT>(create_bitfield_set_on_write_adapter<false>(bitfield_null_adapter<DT>(), bitfield), "<set_on_write>", syncType));
}

template <typename DT, typename C>
inline void set_set_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_write_adapter<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  assert(c);
  bitfield.set_write_callback(create_bitfield_write_callback<DT>(create_bitfield_set_on_write_adapter<false>(create_bitfield_write_adapter<DT>(*c, cb), bitfield), cbName, syncType));
}

template <typename DT, typename C>
inline void set_set_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_untimed_write_adapter<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&bitfield, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << bitfield.get_name() << "'"
...
template <typename DT, typename C>
inline void set_set_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_write_adapter_tagged<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  assert(c);
  bitfield.set_write_callback(create_bitfield_write_callback<DT>(create_bitfield_set_on_write_adapter<false>(create_bitfield_write_adapter<DT>(*c, cb, id), bitfield), cbName, syncType));
}

template <typename DT, typename C>
inline void set_set_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_untimed_write_adapter_tagged<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&bitfield, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << bitfield.get_name() << "'"
...
template <typename DT, typename C>
inline void set_set_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_mask_write_adapter<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  assert(c);
  bitfield.set_write_callback(create_bitfield_write_callback<DT>(create_bitfield_set_on_write_adapter<false>(create_bitfield_write_adapter<DT>(*c, cb), bitfield), cbName, syncType));
}

template <typename DT, typename C>
inline void set_set_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_untimed_mask_write_adapter<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&bitfield, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << bitfield.get_name() << "'"
...
template <typename DT, typename C>
inline void set_set_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_mask_write_adapter_tagged<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  assert(c);
  bitfield.set_write_callback(create_bitfield_write_callback<DT>(create_bitfield_set_on_write_adapter<false>(create_bitfield_write_adapter<DT>(*c, cb, id), bitfield), cbName, syncType));
}

template <typename DT, typename C>
inline void set_set_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_untimed_mask_write_adapter_tagged<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&bitfield, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << bitfield.get_name() << "'"
...
template <typename DT, typename C>
inline void set_post_set_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_post_write_adapter<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&bitfield, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << bitfield.get_name() << "'"
  ...
  assert(c);
  bitfield.set_write_callback(create_bitfield_write_callback<DT>(create_bitfield_set_on_write_adapter<true>(create_bitfield_post_write_adapter<DT>(*c, cb), bitfield), cbName, syncType));
}

template <typename DT, typename C>
inline void set_post_set_on_write_callback(bitfield<DT>& bitfield, C* c, typename bitfield_post_write_adapter_tagged<DT,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&bitfield, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << bitfield.get_name() << "'"
  ...
  assert(c);
  bitfield.set_write_callback(create_bitfield_write_callback<DT>(create_bitfield_set_on_write_adapter<true>(create_bitfield_post_write_adapter<DT>(*c, cb, id), bitfield), cbName, syncType));
}
