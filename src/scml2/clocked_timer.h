   ...
   class SNPS_VP_API clocked_timer : public sc_core::sc_object, public clocked_module {
    public:
	 ...
	 return clocked_module::get_clock();
      ...
      /** Tries to retrieve the value of the internal counter of the timer for
       *  the future time sc_time_stamp()+DELAY. For example if called with
       *  DELAY=SC_ZERO_TIME, it delivers the same value as
       *  get_counter_value(). The value can only be delivered if the provided
       *  delay is shorter than the duration until the timer fires next, in
       *  which case the function returns true and writes the counter value into
       *  the reference argument VALUE. If delay exceeds the next fire time, the
       *  function returns false and does not write into VALUE. The function
       *  will also fail and return false in case it is called from a timer
       *  callback. */
