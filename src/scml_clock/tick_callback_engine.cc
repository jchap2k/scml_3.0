...
namespace n_scml_clock {
   ...
   class TickTrigger {
    ...
    public:
      ...
      : mEvent(
         sc_core::sc_gen_unique_name("clock_tick_event")
      ), mHandle()
      {
...
n_scml_clock::tick_callback_engine::tick_callback_engine(
   scml_clock* master, const sc_core::sc_time& period, const sc_core::sc_time& offset, bool running
    ...
    mMaxClockTicksToSkip(period.value()!=0? (~0ull / period.value()) : ~0ull),
...
n_scml_clock::tick_callback_engine::~tick_callback_engine() {
   {
      ...
      while (node != 0) {
	 scml2::clocked_module* request = node->mRequests;
	 while (request != 0) {
...
void n_scml_clock::tick_callback_engine::handle_tick1_trigger() {
   ...
   mLastTickTime = sc_core::sc_time_stamp();
   mLastTickTimeValid = true;
   ...
   if (mRequestsRoot!=0 && mRequestsRoot->mTickTrigger==0) {
      ...
      unsigned long long delta_ticks = mRequestsRoot->mScheduledTick - mProcessingTickCount;
      if (delta_ticks >= mMaxClockTicksToSkip) {
         SCML2_LOG_WARNING(mMaster, std::cerr, UNDEFINED_WARNING)
            << "Warning in scml_clock \"" << mMaster->name()
            << "\": ignoring tick callback request that is too far into the furure: Multiplication overflow by large clock period or requested ticks to skip." << std::endl
            << "    period = " << mPeriod << std::endl
            << "    ignored request scheduled in " << delta_ticks << " ticks" << std::endl;
      } else {
         sc_core::sc_time delay = RAW_SC_TIME(delta_ticks*mPeriod.value());
         if (delay.value() > ~0ull - sc_core::sc_time_stamp().value()) {
            SCML2_LOG_WARNING(mMaster, std::cerr, UNDEFINED_WARNING)
               << "Warning in scml_clock \"" << mMaster->name()
               << "\": ignoring tick callback request that is too far into the furure: would be scheduled behind max. value of simulation time." << std::endl
               << "    period = " << mPeriod << std::endl
               << "    ignored request scheduled in " << delta_ticks << " ticks" << std::endl;
         } else {
...
void n_scml_clock::tick_callback_engine::request_trigger(scml2::clocked_module* request, long long clock_ticks_to_skip) {
   ...
   unsigned long long delta_ticks = clock_ticks_to_skip + 1;
   ...
   if (request_trigger_impl(request, prev_tick+delta_ticks)) {
      ...
      if (delta_ticks >= mMaxClockTicksToSkip) {
         ...
         unsigned long long tick_time  = prev_tick_time.value() + delta_ticks*mPeriod.value();
         sc_core::sc_time now = sc_core::sc_time_stamp();
         if (tick_time < now.value()) {
            SCML2_LOG_WARNING(mMaster, std::cerr, UNDEFINED_WARNING)
               << "Warning in scml_clock \"" << mMaster->name()
               << "\": ignoring tick callback request that is too far into the furure: would be scheduled behind max. value of simulation time." << std::endl
               << "    period = " << mPeriod << std::endl
            ...
            mRequestsRoot->mTickTrigger->mEvent.notify(RAW_SC_TIME(tick_time) - now);
         }
      }
   }
}
...
void n_scml_clock::tick_callback_engine::request_trigger(scml2::clocked_module* request, const sc_core::sc_time& delay) {
   ...
   if (request_trigger_impl(request, prev_tick+1)) {
      assert(mRequestsRoot!=0 && mRequestsRoot->mTickTrigger==0);
      mRequestsRoot->mTickTrigger = create_tick_trigger();
      sc_core::sc_time delay = prev_tick_time + mPeriod - now;
      mRequestsRoot->mTickTrigger->mEvent.notify(delay);
   }
}
...
void n_scml_clock::tick_callback_engine::cancel_clock_trigger(scml2::clocked_module* request) {
   ...
   if (request->mScheduledTick == mProcessingTickCount) {
      SCML2_LOG_ERROR(mMaster, std::cerr, SCML_INVALID_API_USAGE) << "Fatal error in SCML clock \"" << mMaster->name() << "\": in method cancel_clock_trigger() while processing a clock tick:" << std::endl
		<< "             Canceling a request that is scheduled for the current tick is not permitted!" << std::endl
   ...
   if (mRequestsRoot->mRequests == request) {
	 ...
	 if (!is_processing_tick() && mRunning && mRequestsRoot!=0 && mRequestsRoot->mTickTrigger==0) {
	    ...
	    get_prev_tick_data(tick_count, prev_tick_time);
            ...
            unsigned long long delta_ticks = mRequestsRoot->mScheduledTick - tick_count;
            if (delta_ticks >= mMaxClockTicksToSkip) {
               SCML2_LOG_WARNING(mMaster, std::cerr, UNDEFINED_WARNING)
                  << "Warning in scml_clock \"" << mMaster->name()
                  << "\": ignoring tick callback request that is too far into the furure: Multiplication overflow by large clock period or requested ticks to skip." << std::endl
...
void n_scml_clock::tick_callback_engine::update_parameters(const sc_core::sc_time& next_period, const sc_core::sc_time& next_offset, bool next_running) {
   if (!next_running) { // disabling
	 ...
	 mRunning = false;
      ...
      mPeriod = next_period;
      mOffset = next_offset;
      assert(mPeriod.value() != 0);
      mMaxClockTicksToSkip = ~0ull / mPeriod.value();
      return;
   }
   ...
   if (mRequestsRoot != 0) {
      ...
      unsigned long long delta_ticks = scheduled_tick_count-mLastTickCount-1;
      if (delta_ticks >= mMaxClockTicksToSkip) {
         SCML2_LOG_WARNING(mMaster, std::cerr, UNDEFINED_WARNING)
            << "Warning in scml_clock \"" << mMaster->name()
            << "\": ignoring tick callback request that is too far into the furure: Multiplication overflow by large clock period or requested ticks to skip." << std::endl
         ...
         unsigned long long delta = delta_ticks*mPeriod.value();
         if (delta > ~0ull - mOffset.value()) {
            SCML2_LOG_WARNING(mMaster, std::cerr, UNDEFINED_WARNING)
               << "Warning in scml_clock \"" << mMaster->name()
               << "\": ignoring tick callback request that is too far into the furure: would be scheduled behind max. value of simulation time." << std::endl
               << "    period = " << mPeriod << std::endl
            ...
            mRequestsRoot->mTickTrigger->mEvent.notify(mOffset + RAW_SC_TIME(delta) - sc_core::sc_time_stamp());
         }
      }
   }
}
