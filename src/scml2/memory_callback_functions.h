...
set_read_filter(MEM_OBJECT<DT>& mem, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& filterCBName) {
  SCML2_MODEL_INTERNAL_TO(&mem, SCML_DEPRECATED_API_USAGE) << "Read filter configured on " << mem.get_name() << ". Filters are deprecated and will be removed soon. Please use access restrictions instead." << std::endl;
...
set_write_filter(MEM_OBJECT<DT>& mem, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& filterCBName) {
  SCML2_MODEL_INTERNAL_TO(&mem, SCML_DEPRECATED_API_USAGE) << "Write filter configured on " << mem.get_name() << ". Filters are deprecated and will be removed soon. Please use access restrictions instead." << std::endl;
