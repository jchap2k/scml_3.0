...
{
public:
 bitfield_write_once_adapter(ADAPTER f, bitfield<DT>& b) : mAdapter(f), mBitfield(b), mValue() {
    mWriteOnceState = new write_once_state();
    mWriteOnceState->ref();
  }

 bitfield_write_once_adapter(const bitfield_write_once_adapter& o) : mAdapter(o.mAdapter), mBitfield(o.mBitfield), mValue() {
