...
{
  DT disabled_bit_enables = disable_bit_enables(out_bit_enables, static_cast<DT>(~in_data_mask));
...
{
  store_into_mem_obj(in_data, get_disabled_bit_enables(out_bit_enables, static_cast<DT>(~in_data_mask)), mem_obj);
...
{
  return restrict_some(data, bit_enables, restrict_value, static_cast<DT>(~DT()));
}
...
restrict_all_and_store(DT& data, DT& bit_enables, DT restrict_value, MEM_OBJECT<DT>& mem_obj)
{
 ...
 namespace {
   template<typename DT>
   unsigned char to_uchar(DT value)
   {
     return static_cast<unsigned char>(value);
   }

   template<int BW>
   unsigned char to_uchar(const sc_dt::sc_biguint<BW>& value)
   {
     return static_cast<unsigned char>(value.to_uint());
   }
 }

template<typename DT>
access_restriction_result
restrict_some(tlm::tlm_generic_payload& trans, DT restrict_value = DT(), DT restrict_mask = static_cast<DT>(~DT()))
{
  ...
  for (unsigned i = 0; i != trans.get_data_length(); ++i) {
    ...
    if (be != 0x00) {
      ...
      unsigned char restrict_value_bits = to_uchar(extract_bits(restrict_value, 8, 8*offset));
      unsigned char restrict_mask_bits = to_uchar(extract_bits(restrict_mask, 8, 8*offset));
      data = (restrict_value_bits & (be&restrict_mask_bits)) | (data & ~(be&restrict_mask_bits));
      be &= ~restrict_mask_bits;
    }
