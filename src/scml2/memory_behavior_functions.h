...
void clear_on_write(MEM_OBJECT<DT>& object,
		      const DT& /*data*/,
                      const DT& mask)
{
  object &= ~(mask);
...
template <typename DT, template <typename> class MEM_OBJECT>
void set_on_write(MEM_OBJECT<DT>& object,
		    const DT& /*data*/,
                    const DT& mask)
{
  object |= mask;
...
void clear_on_write(memory_index_reference<DT> object,
                      const DT& data,
                      const DT& mask)
{
  clear_on_write<DT, memory_index_reference>(object, data, mask);
}
...
template <typename DT>
void set_on_write(memory_index_reference<DT> object,
                    const DT& data,
                    const DT& mask)
{
  set_on_write<DT, memory_index_reference>(object, data, mask);
}
