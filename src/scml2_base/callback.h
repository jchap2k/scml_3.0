...
namespace scml2 { namespace base {
	...
	if (m_behavior) {
	  ...
	  SCML2_ERROR_TO(get_parent(), FATAL_ERROR) << "Callback '" << get_name() << "' has not been associated with any behavior." << std::endl;
	  abort();
	}
      };
	...
	return (bool)m_behavior;
      ...
      operator bool() const
      {
	return m_set_behavior_called;
      }
    private:
      bool m_set_behavior_called{};
