...
checkRange(unsigned long long base, unsigned long long size) const
{
  ...
  if (base % get_width()) {
    SCML2_ERROR(SCML_INVALID_API_USAGE) << " to-be-mapped range with base=" << base << " and size=" << size << " is not aligned on router width=" << get_width() << std::endl;
    return false;
  }

  // Mapped size must be a multiple of width
  if (size % get_width()) {
    SCML2_ERROR(SCML_INVALID_API_USAGE) << " to-be-mapped range with base=" << base << " and size=" << size << " is not a multiple of router width=" << get_width() << std::endl;
    return false;
  }

  // Mapped size must fit into size of router object
  if ((base + size) > index_to_address(get_size(), get_width())) {
    SCML2_ERROR(SCML_INVALID_API_USAGE) << " to-be-mapped range with base=" << base << " and size=" << size << " does not fit in size of router=" << index_to_address(get_size(), get_width()) << std::endl;
    return false;
  }
             ...
             unsigned long long size) const
{
  ...
  for (; it != end; ++it) {
    ...
    if ((iterLow <= high) && (iterHigh >= low)) {
      SCML2_ERROR(SCML_INVALID_API_USAGE) << " to-be-mapped range with base=" << base << " and size=" << size << " overlaps with previously mapped range with base=" << range->get_base() << " and size=" << range->get_size() << std::endl;
      return true;
    }
  }
...
bool 
router_base::sPreVReleaseTlmIgnoreBehavior = (::getenv("SNPS_ENABLE_TLM_IGNORE_COMMAND") == nullptr);

mapped_range*
router_base::
getMappedRange(tlm::tlm_generic_payload& trans)
{
  ...
  if (trans.is_read()) {
    ...
    if (sPreVReleaseTlmIgnoreBehavior) {
      return nullptr;
    }  else {
      mapped_range* readRange = getMappedRange(mMappedReadRanges, address);
      mapped_range* writeRange = getMappedRange(mMappedWriteRanges, address);
      if (readRange && writeRange && *readRange == *writeRange) {
	return readRange;
      } else {
	return nullptr;
      }
...
bool 
router_base::
get_forward_unmapped_ignore_accesses_into_callback() const
{
  return mForwardUnmappedIgnoreAccessesIntoCallback;
}

void 
router_base::
set_forward_unmapped_ignore_accesses_into_callback(bool value)
{
  mForwardUnmappedIgnoreAccessesIntoCallback = value;
}
...
{
  if (trans.get_streaming_width() == 0 && trans.get_command() != tlm::TLM_IGNORE_COMMAND) {
    SCML2_ERROR(GENERIC_ERROR) << "transaction streaming width is set to zero." << std::endl;
  }
  scml2::tlm2::payload_scope_helper psh(trans);
  ...
  unsigned step = std::min(trans.get_data_length(), trans.get_streaming_width());
  bool is_empty_ignore = trans.get_command() == tlm::TLM_IGNORE_COMMAND && step == 0;
  const unsigned long long endAddress = address + (is_empty_ignore ? 0: (step - 1));
  const unsigned long long highIndex = address_to_index(endAddress, get_width());

  if (mapped_range* range = getMappedRange(trans)) {
    if ((lowIndex != highIndex) && !range->contains(endAddress) && trans.get_command() != tlm::TLM_IGNORE_COMMAND /* never attempt to unroll ignore command */) {
    ...
    if (highIndex >= get_size()) {
      SCML2_ERROR(ACCESS_UNMAPPED_ADDRESS) << "Access to address 0x" << std::hex << address << "-0x" << endAddress << std::dec << " is out of bounds." << std::endl;
      trans.set_response_status(tlm::TLM_ADDRESS_ERROR_RESPONSE);
    } else if (trans.get_command() == tlm::TLM_IGNORE_COMMAND &&
	       !mForwardUnmappedIgnoreAccessesIntoCallback) {
      if (sPreVReleaseTlmIgnoreBehavior) {
	trans.set_response_status(tlm::TLM_OK_RESPONSE);
      } else {
	SCML2_ERROR(ACCESS_UNMAPPED_ADDRESS) << "Ignore access to address 0x" << std::hex << address << "-0x" << endAddress << std::dec << " is undecodable." << std::endl;
...
get_direct_mem_ptr(tlm::tlm_generic_payload& trans, tlm::tlm_dmi& dmiData)
{
  ...
  static bool force_allow_rw_flag_in_router = ::getenv("SNPS_VP_FORCE_ALLOW_RW_FLAG_IN_ROUTER") != 0;
  if (force_allow_rw_flag_in_router) {
    if (trans.is_read()) {
      dmiData.allow_read();
    } else {
      dmiData.allow_write();
  ...
  for (; rangeIter != end; ++rangeIter) {
    ...
    if(range->contains(address)) {
      ...
      if (rangeIter != ranges.begin()) {
        // keep ranges in MRU order
        ranges.splice(ranges.begin(), ranges, rangeIter);
      }
      return result;
...
transport_dbg(tlm::tlm_generic_payload& trans)
{
  const unsigned long long address = trans.get_address();
  const unsigned long long lowIndex = address_to_index(address, get_width());
  bool is_empty_ignore = trans.get_command() == tlm::TLM_IGNORE_COMMAND && trans.get_data_length() == 0;
  const unsigned long long endAddress = address + 
    (is_empty_ignore ? 0 : (trans.get_data_length() - 1));
  const unsigned long long highIndex = address_to_index(endAddress, get_width());
