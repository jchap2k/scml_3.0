...
#define SCML2_BASE_CALLBACK_IGNORE(NAME, RET, ...)                        \
  scml2::base::callback<RET, ## __VA_ARGS__ > NAME{#NAME, [](__VA_ARGS__) -> RET { typedef RET return_type; return return_type(); }}; \
  template<typename Model>                                                \
  void set_ ## NAME ## _callback(Model* m, typename scml2::base::callback_impl<Model, RET, ## __VA_ARGS__ >::callback_type cb, const std::string& n) { \
    NAME.set_behavior(scml2::base::callback_impl<Model, RET, ## __VA_ARGS__ >(n, *m, cb)); \
  };                                                                      \
...
#define SCML2_BASE_CALLBACK_IGNORE_W_VAL(NAME, RET, RET_V, ...)                                                             \
  scml2::base::callback<RET, ## __VA_ARGS__ > NAME{#NAME, [](__VA_ARGS__) -> RET { typedef RET return_type; return return_type(RET_V); }}; \
  template<typename Model>                                                                                    \
  void set_ ## NAME ## _callback(Model* m, typename scml2::base::callback_impl<Model, RET, ## __VA_ARGS__ >::callback_type cb, const std::string& n) { \
    NAME.set_behavior(scml2::base::callback_impl<Model, RET, ## __VA_ARGS__ >(n, *m, cb)); \
  };                                                                      \
...
#define SCML2_BASE_CALLBACK_WARNING(NAME, RET, ...)                                \
  scml2::base::callback<RET, ## __VA_ARGS__ > NAME{#NAME, [this](__VA_ARGS__) -> RET { SCML2_WARNING_TO(this, ACCESS_IGNORED) << "call " << #NAME << " ignored in " << this->get_name() << std::endl; typedef RET return_type; return return_type(); }}; \
  template<typename Model>                                                \
  void set_ ## NAME ## _callback(Model* m, typename scml2::base::callback_impl<Model, RET, ## __VA_ARGS__ >::callback_type cb, const std::string& n) { \
    NAME.set_behavior(scml2::base::callback_impl<Model, RET, ## __VA_ARGS__ >(n, *m, cb)); \
  };                                                                      \
