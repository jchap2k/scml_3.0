...
void n_scml_clock::divider_configurator::init() {
   ...
   if (mClkInPort == 0) {
      if (mMaster == 0) {
         SCML2_LOG_ERROR(mEngine, std::cerr, CONFIGURATION_ERROR) << "Error in scml_clock '" << mEngine->name() << "': master clock is unbound! A divider needs to be boud to a master clock before end_of_elaboration!" << std::endl;
         abort();
      }
      mInitialized = true;
      mMaster->init();
