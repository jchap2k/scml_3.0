...
#undef HELPER
    ...
    public:
      ...
      {
	std::string args_str = SCML2_MODEL_INTERNAL_ENABLED(m_object, LEVEL6) ? impl::to_string(args...) : "";
	Ret ret = f(args...);
	SCML2_MODEL_INTERNAL_TO(m_object, LEVEL6) << "Callback '" << n.get_name() << "' returned " << impl::to_string(ret) << (args_str.length() > 0 ? ", called with args: " : "' called") << args_str << std::endl;
	...
	std::string args_str = SCML2_MODEL_INTERNAL_ENABLED(m_object, LEVEL6) ? impl::to_string(args...) : "";
	f(args...);
	SCML2_MODEL_INTERNAL_TO(m_object, LEVEL6) << "Callback '" << n.get_name() << (args_str.length() > 0 ? "' called with args: " : "' called") << args_str << std::endl;
	for (auto o : this->m_observers) {
