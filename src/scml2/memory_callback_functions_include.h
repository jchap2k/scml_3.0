...
inline void set_read_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_transport_adapter<C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
...
template <typename DT, template <typename> class MEM_OBJECT>
inline void set_clear_on_write(MEM_OBJECT<DT>& mem, SyncType syncType  = AUTO_SYNCING) {
  mem.set_write_callback(create_memory_callback(create_clear_on_write_adapter<false>(null_adapter(), mem), "<clear_on_write>", syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename FC>
inline void set_clear_on_write(MEM_OBJECT<DT>& mem, SyncType syncType , FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */) {
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_clear_on_write_adapter<false>(null_adapter(), mem), *filterClass, filterCB), "<clear_on_write>", syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename transport_adapter<C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_clear_on_write_adapter<false>(create_transport_adapter(*c, cb), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename transport_adapter<C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_clear_on_write_adapter<false>(create_transport_adapter(*c, cb), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_transport_adapter<C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_transport_adapter<C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename transport_adapter_tagged<C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_clear_on_write_adapter<false>(create_transport_adapter(*c, cb, id), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename transport_adapter_tagged<C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_clear_on_write_adapter<false>(create_transport_adapter(*c, cb, id), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_transport_adapter_tagged<C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_transport_adapter_tagged<C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename write_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_clear_on_write_adapter<false>(create_word_access_unroller<DT>(create_write_adapter(*c, cb, mem)), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename write_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_clear_on_write_adapter<false>(create_word_access_unroller<DT>(create_write_adapter(*c, cb, mem)), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename write_extensions_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_clear_on_write_adapter<false>(create_word_access_unroller<DT>(create_write_adapter(*c, cb, mem)), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename write_extensions_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_clear_on_write_adapter<false>(create_word_access_unroller<DT>(create_write_adapter(*c, cb, mem)), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_write_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_write_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_write_extensions_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_write_extensions_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename write_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_clear_on_write_adapter<false>(create_word_access_unroller<DT>(create_write_adapter(*c, cb, id, mem)), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename write_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_clear_on_write_adapter<false>(create_word_access_unroller<DT>(create_write_adapter(*c, cb, id, mem)), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename write_extensions_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_clear_on_write_adapter<false>(create_word_access_unroller<DT>(create_write_adapter(*c, cb, id, mem)), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename write_extensions_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_clear_on_write_adapter<false>(create_word_access_unroller<DT>(create_write_adapter(*c, cb, id, mem)), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_write_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_write_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_write_extensions_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_write_extensions_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_word_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename word_write_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING, bool check_word_access = true) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_clear_on_write_adapter<false>(create_word_write_adapter(*c, cb, check_word_access, mem), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_word_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename word_write_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, bool check_word_access = true) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_clear_on_write_adapter<false>(create_word_write_adapter(*c, cb, check_word_access, mem), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_word_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename word_write_extensions_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING, bool check_word_access = true) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_clear_on_write_adapter<false>(create_word_write_adapter(*c, cb, check_word_access, mem), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_word_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename word_write_extensions_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, bool check_word_access = true) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_clear_on_write_adapter<false>(create_word_write_adapter(*c, cb, check_word_access, mem), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_word_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_word_write_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING, bool check_word_access = true) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_word_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_word_write_extensions_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING, bool check_word_access = true) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_word_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename word_write_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id, bool check_word_access = true) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_clear_on_write_adapter<false>(create_word_write_adapter(*c, cb, id, check_word_access, mem), mem), cbName, syncType));
}
...
inline void set_word_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename word_write_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id, bool check_word_access = true) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_clear_on_write_adapter<false>(create_word_write_adapter(*c, cb, id, check_word_access, mem), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_word_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename word_write_extensions_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id, bool check_word_access = true) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_clear_on_write_adapter<false>(create_word_write_adapter(*c, cb, id, check_word_access, mem), mem), cbName, syncType));
}
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_word_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_word_write_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id, bool check_word_access = true) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_word_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_word_write_extensions_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id, bool check_word_access = true) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_post_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename post_write_adapter<C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
  ...
  assert(c);
  mem.set_write_callback(create_memory_callback(create_clear_on_write_adapter<true>(create_post_write_adapter(*c, cb), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_post_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename post_write_adapter<C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
  ...
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_clear_on_write_adapter<true>(create_post_write_adapter(*c, cb), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_post_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename post_write_adapter_tagged<C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
  ...
  assert(c);
  mem.set_write_callback(create_memory_callback(create_clear_on_write_adapter<true>(create_post_write_adapter(*c, cb, id), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_post_clear_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename post_write_adapter_tagged<C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
  ...
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_clear_on_write_adapter<true>(create_post_write_adapter(*c, cb, id), mem), *filterClass, filterCB), cbName, syncType));
}
...
template <typename DT, template <typename> class MEM_OBJECT>
inline void set_set_on_write(MEM_OBJECT<DT>& mem, SyncType syncType  = AUTO_SYNCING) {
  mem.set_write_callback(create_memory_callback(create_set_on_write_adapter<false>(null_adapter(), mem), "<set_on_write>", syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename FC>
inline void set_set_on_write(MEM_OBJECT<DT>& mem, SyncType syncType , FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */) {
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_set_on_write_adapter<false>(null_adapter(), mem), *filterClass, filterCB), "<set_on_write>", syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename transport_adapter<C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_set_on_write_adapter<false>(create_transport_adapter(*c, cb), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename transport_adapter<C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_set_on_write_adapter<false>(create_transport_adapter(*c, cb), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_transport_adapter<C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_transport_adapter<C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename transport_adapter_tagged<C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_set_on_write_adapter<false>(create_transport_adapter(*c, cb, id), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename transport_adapter_tagged<C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_set_on_write_adapter<false>(create_transport_adapter(*c, cb, id), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_transport_adapter_tagged<C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_transport_adapter_tagged<C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename write_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_set_on_write_adapter<false>(create_word_access_unroller<DT>(create_write_adapter(*c, cb, mem)), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename write_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_set_on_write_adapter<false>(create_word_access_unroller<DT>(create_write_adapter(*c, cb, mem)), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename write_extensions_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_set_on_write_adapter<false>(create_word_access_unroller<DT>(create_write_adapter(*c, cb, mem)), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename write_extensions_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_set_on_write_adapter<false>(create_word_access_unroller<DT>(create_write_adapter(*c, cb, mem)), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_write_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_write_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_write_extensions_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_write_extensions_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename write_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_set_on_write_adapter<false>(create_word_access_unroller<DT>(create_write_adapter(*c, cb, id, mem)), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename write_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_set_on_write_adapter<false>(create_word_access_unroller<DT>(create_write_adapter(*c, cb, id, mem)), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename write_extensions_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_set_on_write_adapter<false>(create_word_access_unroller<DT>(create_write_adapter(*c, cb, id, mem)), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename write_extensions_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_set_on_write_adapter<false>(create_word_access_unroller<DT>(create_write_adapter(*c, cb, id, mem)), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_write_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_write_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_write_extensions_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_write_extensions_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_word_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename word_write_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING, bool check_word_access = true) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_set_on_write_adapter<false>(create_word_write_adapter(*c, cb, check_word_access, mem), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_word_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename word_write_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, bool check_word_access = true) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_set_on_write_adapter<false>(create_word_write_adapter(*c, cb, check_word_access, mem), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_word_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename word_write_extensions_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING, bool check_word_access = true) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_set_on_write_adapter<false>(create_word_write_adapter(*c, cb, check_word_access, mem), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_word_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename word_write_extensions_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, bool check_word_access = true) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_set_on_write_adapter<false>(create_word_write_adapter(*c, cb, check_word_access, mem), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_word_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_word_write_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING, bool check_word_access = true) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_word_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_word_write_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, bool check_word_access = true) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_word_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_word_write_extensions_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING, bool check_word_access = true) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_word_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_word_write_extensions_adapter<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, bool check_word_access = true) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_word_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename word_write_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id, bool check_word_access = true) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_set_on_write_adapter<false>(create_word_write_adapter(*c, cb, id, check_word_access, mem), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_word_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename word_write_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id, bool check_word_access = true) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_set_on_write_adapter<false>(create_word_write_adapter(*c, cb, id, check_word_access, mem), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_word_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename word_write_extensions_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id, bool check_word_access = true) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_set_on_write_adapter<false>(create_word_write_adapter(*c, cb, id, check_word_access, mem), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_word_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename word_write_extensions_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id, bool check_word_access = true) {
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_set_on_write_adapter<false>(create_word_write_adapter(*c, cb, id, check_word_access, mem), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_word_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_word_write_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id, bool check_word_access = true) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_word_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_word_write_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id, bool check_word_access = true) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_word_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_word_write_extensions_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id, bool check_word_access = true) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_word_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename untimed_word_write_extensions_adapter_tagged<MEM_OBJECT<DT>,C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id, bool check_word_access = true) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
...
template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_post_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename post_write_adapter<C>::CallbackType cb, const std::string& cbName, SyncType syncType = AUTO_SYNCING) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
  ...
  assert(c);
  mem.set_write_callback(create_memory_callback(create_set_on_write_adapter<true>(create_post_write_adapter(*c, cb), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_post_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename post_write_adapter<C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
  ...
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_set_on_write_adapter<true>(create_post_write_adapter(*c, cb), mem), *filterClass, filterCB), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C>
inline void set_post_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename post_write_adapter_tagged<C>::CallbackType cb, const std::string& cbName, SyncType syncType, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
  ...
  assert(c);
  mem.set_write_callback(create_memory_callback(create_set_on_write_adapter<true>(create_post_write_adapter(*c, cb, id), mem), cbName, syncType));
}

template <typename DT, template <typename> class MEM_OBJECT, typename C, typename FC>
inline void set_post_set_on_write_callback(MEM_OBJECT<DT>& mem, C* c, typename post_write_adapter_tagged<C>::CallbackType cb, const std::string& cbName, SyncType syncType, FC* filterClass, bool (FC::*filterCB)(tlm::tlm_generic_payload&, sc_core::sc_time&), const std::string& /* filterCBName */, int id) {
  if (syncType == SELF_SYNCING) {
    SCML2_ERROR_TO(&mem, CONFIGURATION_ERROR)
      << "SELF_SYNCING not supported for untimed callbacks on '"
      << mem.get_name() << "'"
  ...
  assert(c);
  mem.set_write_callback(create_memory_callback(create_filter_adapter(create_set_on_write_adapter<true>(create_post_write_adapter(*c, cb, id), mem), *filterClass, filterCB), cbName, syncType));
}
