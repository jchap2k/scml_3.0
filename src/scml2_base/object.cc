...
namespace scml2 { namespace base {
    ...
    void object::add_self_to_registry(const object_name& name)
    {
    ...
    object::object(const object_name& name)
      : atom()
      , scml2::tagged_message_handler_provider(sc_core::sc_get_current_object())
      , m_children()
      , m_finalized(false)
    {
      add_self_to_registry(name);
    }
    ...
    {
      if (m_finalized) {
	SCML2_ERROR(SCML_INVALID_API_USAGE) << "scml2::base::object '" << get_name() << "' is being finalized multiple times." << std::endl;
      }
      in_construction_scope([&](){finalize_local_construction();});
      for (auto child : m_children) {
