...
reg(const std::string& name,
  ...
  mToplevel(parent),
  mFastAccess(),
  mCallbackDefaults(mToplevel, mFastAccess),
...
setup()
{
  ...
  init_default_callback(&mCallbackDefaults);
  init_default_debug_callback(&mCallbackDefaults);
  init_default_restriction(&mCallbackDefaults);
...
post_filter(tlm::tlm_generic_payload& trans) const
{
  ...
  if (!mBitfieldContainer.empty()) {
    // NOTE! order of these two helper instances is important: be needs to be destroyed after data; otherwise orig byte-enabled will be overwritten and intended data will not be copied back
    array_from_dt<DT> be  (trans.get_byte_enable_ptr(), sizeOf<DT>(), 0, false /*copy back in destructor*/, trans.get_byte_enable_ptr());
    array_from_dt<DT> data(trans.get_data_ptr(),        sizeOf<DT>(), 0, false /*copy back in destructor*/, trans.get_byte_enable_ptr());
    if (mBitfieldContainer.filter(trans.is_read(), (DT&)data, (DT&)be)) {
