...
bool 
toplevel_memory_base::
s_dont_ignore_already_filtered = (::getenv("SNPS_SLS_SCML2_DONT_IGNORE_ALREADY_FILTERED") == 0);
...
toplevel_memory_base(const std::string& name,
  ...
  mReadDmiData(size, this),
  mWriteDmiData(size, this),
...
invalidate_dmi()
{
  if (mDmiPointersInUse && !sc_core::sc_end_of_simulation_invoked()) {
...
namespace {
  std::string byte_array_to_str(unsigned char* data, unsigned length) {
    std::ostringstream os;
    for (unsigned int i = 0; i < length; ++i) {
      os << " " << std::setw(2) << std::setfill('0') << std::hex << (int)(data[i]);
    }
    return os.str();
  }
}
...
public:
    ...
    , m_filter_error(false)
  {
	...
	unsigned char* old_be_ptr = trans.get_byte_enable_ptr();
	unsigned m_old_be_length = trans.get_byte_enable_length();
	m_be_helper.engage(&m_filtered_byte_enables[0]);
	if (SCML2_MODEL_INTERNAL_ENABLED(&m_mem, LEVEL6)) {
	  bool any_different = false;
	  bool any_still_left = false;
	  for (unsigned i = 0; i != trans.get_data_length(); ++i) {
	    unsigned char old_be = (old_be_ptr != nullptr) ? old_be_ptr[i%m_old_be_length] : 0xff;
	    any_different |= (trans.get_byte_enable_ptr()[i] != old_be);
	    any_still_left |= (trans.get_byte_enable_ptr()[i] != 0);
	  }
	  if (any_different) {
	    SCML2_MODEL_INTERNAL_TO(&m_mem, LEVEL6) << m_mem.basename() << " " << (trans.is_read() ? "read" : "write") << " " << (any_still_left ? "partially" : "completely") << " restricted for relative address 0x" << std::hex << trans.get_address() << " (size: 0x" << trans.get_data_length() << (any_still_left ? ", bit-enable-array: " + byte_array_to_str(trans.get_byte_enable_ptr(), trans.get_data_length()) : std::string()) << ")" << std::endl;
	  }
	}
      } else {
	      SCML2_MODEL_INTERNAL_TO(&m_mem, LEVEL6) << m_mem.basename() << " " << (trans.is_read() ? "read" : "write") << " error at relative address 0x" << std::hex << trans.get_address() << " (size: 0x" << trans.get_data_length() << ") because of access restriction" << std::endl;
      }
    }
...
b_transport(tlm::tlm_generic_payload& trans, sc_core::sc_time& t)
{
  ...
  if (!alias) {
    if (lowIndex != highIndex) {
      ...
      SCML2_ERROR(ACCESS_UNMAPPED_ADDRESS) << "Access to address 0x" << std::hex << address << std::dec << " is not allowed." << std::endl;
...
{
  auto relative_alias_size = alias->get_size()*alias->get_width()/get_width();
  ...
  if (alias->is_dmi_allowed()) {
    ...
    mWriteDmiData.disable(alias->get_offset(),
                          alias->get_offset() + relative_alias_size - 1);
    ...
    unsigned long long endIndex = alias->get_offset() + relative_alias_size - 1;
...
get_direct_mem_ptr(tlm::tlm_generic_payload& trans, tlm::tlm_dmi& dmi_data)
{
  ...
  if (trans.is_read()) {
    dmi_data.allow_read();
    dmi_data.set_read_latency(get_default_read_latency() / (get_old_style_dmi_latency() ? 1. : static_cast<double>(get_size())));
  } else {
    dmi_data.allow_write();
    dmi_data.set_write_latency(get_default_write_latency() / (get_old_style_dmi_latency() ? 1. : static_cast<double>(get_size())));
  }
