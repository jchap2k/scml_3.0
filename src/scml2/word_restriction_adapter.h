...
operator()(tlm::tlm_generic_payload& trans)
{
  ...
  auto be = array_from_dt<data_type>(trans.get_byte_enable_ptr(), sizeOf<data_type>(), 0, false/*copy-back*/);
  data_type& orig_bit_enables = be;
  data_type bit_enables = orig_bit_enables;
  auto d = array_from_dt<data_type>(trans.get_data_ptr(), sizeOf<data_type>(), 0, false /*copy-back*/, trans.get_byte_enable_ptr());;
  data_type& orig_data = d;
  data_type data = orig_data;
  assert(mRestriction);
