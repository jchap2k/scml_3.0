...
#if (!defined CWR_SYSTEMC && defined SYSTEMC_VERSION && SYSTEMC_VERSION <= 20221128 /*2.3.4*/)
#elif (defined CWR_SYSTEMC)
  sc_core::sc_object_host* host = sc_get_curr_simcontext()->hierarchy_curr();
  mModule = dynamic_cast<sc_core::sc_module*>(host);
  assert(mModule != nullptr || host == nullptr);
#else
...
\
scml_property< ptype >::scml_property( const scml_property< ptype >& other)\
  : scml_property_base(other.mName) {\
  (*this) = other.getValue();	       \
}\
\
scml_property< ptype >::this_reference_type \
...
enum scml_property_type_enum {
  SCML_PROPERTY_TYPE_INT = 1,
  SCML_PROPERTY_TYPE_UINT,
  SCML_PROPERTY_TYPE_LONGLONG,
  SCML_PROPERTY_TYPE_ULONGLONG,
  SCML_PROPERTY_TYPE_DOUBLE,
  SCML_PROPERTY_TYPE_BOOL,
  SCML_PROPERTY_TYPE_STRING,
};
extern "C"
SNPS_VP_API void * gdb_get_scml_property_value(void *p, scml_property_type_enum t)
{
  switch(t) {
  case SCML_PROPERTY_TYPE_INT:
    {
      scml_property< int > *prop = static_cast<scml_property< int > *>(p);
      return &prop->getValue();
    }
  case SCML_PROPERTY_TYPE_UINT:
    {
      scml_property< unsigned int > *prop = static_cast<scml_property< unsigned int > *>(p);
      return &prop->getValue();
    }
  case SCML_PROPERTY_TYPE_LONGLONG:
    {
      scml_property< long long > *prop = static_cast<scml_property< long long > *>(p);
      return &prop->getValue();
    }
  case SCML_PROPERTY_TYPE_ULONGLONG:
    {
      scml_property< unsigned long long > *prop = static_cast<scml_property< unsigned long long > *>(p);
      return &prop->getValue();
    }
  case SCML_PROPERTY_TYPE_DOUBLE:
    {
      scml_property< double > *prop = static_cast<scml_property< double > *>(p);
      return &prop->getValue();
    }
  case SCML_PROPERTY_TYPE_BOOL:
    {
      scml_property< bool > *prop = static_cast<scml_property< bool > *>(p);
      return &prop->getValue();
    }
  case SCML_PROPERTY_TYPE_STRING:
    {
      scml_property< string > *prop = static_cast<scml_property< string > *>(p);
      return &prop->getValue();
    }
  default:
    return nullptr;
