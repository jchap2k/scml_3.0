...
public:
  ...
  bool operator==(const mapped_range& other) const;
private:
			  ...
			  unsigned long long endRange)
{
  ...
  unsigned long long offsetAndClippedEndRange = (std::min)(endRange-mOffset, mSize-1);
...
bool 
mapped_range::
operator==(const mapped_range& other) const
{
  return get_base() == other.get_base() && get_size() == other.get_size() && 
  get_offset() == other.get_offset() && &mDest == &other.mDest && &mBwIf == &other.mBwIf;
}
